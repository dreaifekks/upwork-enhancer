...
Data is used to provide local job scoring, opportunity review panels, saved decisions, extension settings, and optional AI-assisted analysis.

The extension does not automatically submit proposals, apply to jobs, collect Upwork passwords, proxy Upwork sessions, or crawl Upwork pages in the background.

## Data Sharing

The developer does not sell user data.

The extension does not send data to the developer's own server. When the optional AI feature is enabled, data is sent only to the API endpoint configured by the user. That provider's own terms and privacy policy may apply.
...
The extension uses browser extension APIs and HTTPS API endpoints configured by the user. Users should only configure trusted API endpoints.
